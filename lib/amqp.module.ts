import { DynamicModule, Global, Inject, Module, OnApplicationBootstrap, OnApplicationShutdown, OnModuleDestroy, Provider } from '@nestjs/common'
...
import { getAMQPPubChannelToken, getAMQPConnectionToken, getAMQPSubChannelToken } from './amqp.utils'
...
export class AMQPModule implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(private readonly explorer: AMQPExplorer, @Inject(AMQP_MODULE_OPTIONS) private readonly moduleOptions: AMQPModuleOptions, private readonly moduleRef: ModuleRef) { }
  ...
  static forRoot(connectionOptions: AMQPModuleOptions): DynamicModule {
    ...
    const amqpPubChannelProvider: Provider = {
      ...
      useFactory: async (): Promise<ChannelWrapper> => {
        const ch = amqpConnection.createChannel({
          setup: (ch: Channel) => ch.on('error', (e) => {
    ...
    return {
      module: AMQPModule,
      providers: [amqpConnectionProvider, amqpModuleOptions, amqpPubChannelProvider, amqpSubChannelProvider],
    ...
    await channel.addSetup(async (channel: Channel) => {
      ...
      for (const consumer of consumers) {
        ...
        if (this.moduleOptions.service) {
          ...
          if (!exchange) {
            this.logger.error(`Exchange on queue handler "${consumer.methodName.toString()}" is required since AMQPModule configuration is missing the service property.`)
            throw new Error()
          }
        }

        // handler.assertQueue takes precedence over assertQueuesByDefault
        if (consumer.assertQueue === true || (this.moduleOptions.assertQueuesByDefault === true && consumer.assertQueue !== false)) {
          ...
          channel.bindQueue(
            queueName,
            exchange,
            `${consumer.queueName ?? consumer.pattern}`,
          )
        }
        ...
        this.logger.log(
          `Mapped function ${consumer.methodName.toString()} with queue ${queueName}`,
        )
      }
  ...
  async onApplicationShutdown() {
    const connection = this.moduleRef.get<AmqpConnectionManager>(getAMQPConnectionToken(this.moduleOptions.name))

    const subChannel = this.moduleRef.get<AmqpConnectionManager>(getAMQPPubChannelToken(this.moduleOptions.name))
    const pubChannel = this.moduleRef.get<AmqpConnectionManager>(getAMQPSubChannelToken(this.moduleOptions.name))
